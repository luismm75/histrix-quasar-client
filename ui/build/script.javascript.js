...
function addAssets(builds, type, injectName) {
  ...
  const plugins = [buble(/* bubleConfig */)];
...
function buildEntry(config) {
  return rollup
    ...
    .then(({ output }) => {
      const code = config.rollup.output.format === 'umd' ? injectVueRequirement(output[0].code) : output[0].code;
    ...
    .then((code) => {
      ...
      return buildUtils.writeFile(
        config.build.minExt === true ? addExtension(config.rollup.output.file) : config.rollup.output.file,
